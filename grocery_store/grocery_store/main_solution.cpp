...
struct grocery_item {
  grocery_item()
  : grocery_item("", 0, 0.00, false) {}
  
  grocery_item(const string& name, size_t code, double price, bool onSale)
  : name_(name), code_(code), price_(price), onSale_(onSale) {}
...
//                return lhs.price_ > rhs.price_; });    // gb CANNOT be const if sorting
    sort(gb.items_.begin(), gb.items_.end());
    os << "==== Grocery Bill ========\n";
